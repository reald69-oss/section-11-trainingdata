...
DFA a1 Profile — evaluate these branches IN ORDER and take the first that matches:
  ...
  2. NORMAL CONTENT AVAILABLE → render the full section below: easy_guard_estimate present, OR (trailing_by_sport.cycling.confidence in {moderate, high} AND at least one of lt1_estimate / lt2_estimate present).
  3. DIAGNOSTIC FALLBACK (v3.122) → all three estimates null AND at least one of easy_guard_crossing_sessions / lt1_crossing_sessions / lt2_crossing_sessions ≥1. Crossings that sustained a dwell but failed estimate eligibility are real evidence and must not disappear silently. Render ONLY the sessions line plus the two lines below — NO average/drift line, NO guard estimate, NO dossier values, NO calibration deltas. This is a depth/data-quality diagnostic, not a pseudo-estimate:
