...
After `--init`, the bootstrap `sync.py` in the workspace root is removed — it's now at `section11/examples/sync.py`. All commands from here use the repo copy:
...
Key principle: `section11/` is the repo, exactly as it appears on GitHub. Your personal files live at the workspace root. Updates replace files inside `section11/` — your dossier, config, and data are never touched.
...
Point your agent's workspace at `~/training-data/`. The agent sees personal files (dossier, JSON data) at the root, and the full protocol, templates, and tools in `section11/`.

### OpenClaw

Set workspace to `~/training-data/`. The Section 11 skill reads files directly from the workspace. Configure `HEARTBEAT.md` data sources with local file paths instead of URLs.
...
2. Set your workspace inside the synced folder:
   ```
...
Only changed files are downloaded. Personal files at the workspace root (dossier, config, data) are never touched. Nothing changes without your confirmation — same principle as `--confirm` on push.py writes.
...
**`.sync_config.json` must be in the working directory** — the workspace root (`~/training-data/`), not inside `section11/`. If sync.py can't find credentials, check that your timer's `WorkingDirectory` is set correctly.
